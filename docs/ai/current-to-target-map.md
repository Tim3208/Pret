...
| `src/BattleCombat.tsx` | `src/widgets/battle-stage/ui/BattleStage.tsx` + `src/widgets/battle-stage/lib/*` + `src/features/battle-command-input/*` | 1차 이동 완료 | 위젯 및 lib 이동, 입력 feature 분리, 자원 패널 조합 분리 완료. 오버레이/로그 추가 분해 잔여 |
...
1. `widgets/battle-stage/ui/BattleStage.tsx`에서 오버레이/로그 관련 추가 분해
2. 미사용 파일 정리
