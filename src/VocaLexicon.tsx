...
export default function VocaLexicon({ language, decipher, learnedEntryIds }: VocaLexiconProps) {
  ...
  const headerRule = useMemo(() => makeSectionRule(`${text.pageLabel} ${String(currentPage + 1).padStart(2, "0")} / ${String(pageCount).padStart(2, "0")}`, 28), [currentPage, pageCount, text.pageLabel]);
  ...
  useEffect(() => {
    setBookPosition((current) => clampBookPosition(current, open, viewport, panelWidth, panelHeight));
  }, [open, panelHeight, panelWidth, viewport]);

  useEffect(() => {
    if (pageByCategory[activeCategory] <= pageCount - 1) {
      return;
    }

    setPageByCategory((current) => ({
      ...current,
      [activeCategory]: Math.max(0, pageCount - 1),
    }));
  }, [activeCategory, pageByCategory, pageCount]);
  ...
  const handleTabSelect = useCallback((category: LexiconCategory) => {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[90] font-crt select-none">
      {open && (
        ...
        >
          <div
            ...
            onPointerDown={beginDrag.bind(null, "panel")}
      ...
      <button
        ...
        onPointerDown={beginDrag.bind(null, "book")}
        onPointerMove={moveDrag}
        onPointerUp={finishDrag}
        onPointerCancel={finishDrag}
      >
